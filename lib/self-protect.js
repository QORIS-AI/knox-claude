...
function extractTargets(command) {
  ...
  for (const sub of subs) {
    ...
    if (m) {
      ...
      const argMatch = args.match(/(\S*(?:~|\.claude|\.knox|knox-check|run-check|patterns\.json|lib\/check|policies\/patterns|audit\.log|audit\.jsonl)\S*)/g);
    ...
    if (sedM) {
      const argMatch = sedM[1].match(/(\S*(?:\.claude|knox|settings\.json|patterns\.json)\S*)/g);
      if (argMatch) for (const a of argMatch) targets.push(a);
    }
...
function checkVariableIndirection(command) {
  const DANGEROUS_TOKENS = /^(?:rm|curl|wget|nc|ncat|sudo|bash|sh|zsh|dd|mkfs|xmrig)$/i;
