...
function checkCommand(command, config) {
  ...
  // Soft-disable: skip blocklist, parsers, script inspection, sudo sanitization.
  // Self-protection (above) and audit logging (in caller) remain active.
...
function checkReadPath(filePath, config) {
  if (config && config.preset === 'disabled') return null;
...
// Knox self-protection write paths — ALWAYS blocked regardless of preset.
// An agent at preset=disabled shouldn't be able to overwrite Knox's own
// config to lock the user out of ever re-enabling enforcement.
const KNOX_SELFPROTECT_EXACT = ['.knox.json', '.knox.local.json'];
const KNOX_SELFPROTECT_PREFIX = ['.knox/', '.claude/settings'];
...
function checkWritePath(filePath, config) {
  ...
  // Self-protection: Knox config files always blocked, even at preset=disabled
  // and even when write_path_protection is in disabled_checks. These rules
  // are unconditional, mirroring the Bash self-protection in checkCommand.
  for (const e of KNOX_SELFPROTECT_EXACT) {
    if (basename === e || filePath === e) {
      return { blocked: true, critical: true, reason: `Knox: Write to ${e} blocked (self-protection)` };
    }
  }
  for (const pfx of KNOX_SELFPROTECT_PREFIX) {
    if (filePath.startsWith(pfx) || basename.startsWith(pfx)) {
      return { blocked: true, critical: true, reason: `Knox: Write to ${pfx}* blocked (self-protection)` };
    }
  }
